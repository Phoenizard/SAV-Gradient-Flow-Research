"""Phase 1 — Fig 2 reproduction: Example 1 (sin+cos), λ=0.

Paper settings: D=40, m=1000, M=10000, l=64, lr=0.2, C=1, λ=0, 10000 epochs
Methods: PM-Euler (SGD), Vanilla SAV, Restart SAV, Relax SAV

Usage:
    python experiments/phase1_fig2.py              # full 10k epochs
    python experiments/phase1_fig2.py --smoke      # smoke test (100 epochs)
"""
...
# Paper's Fig 2 settings
D = 40
M_TOTAL = 10000
M_NEURONS = 1000
BATCH_SIZE = 64
LR = 0.2  # dt for SAV, lr for SGD
C = 1.0
LAMBDA = 0.0
FULL_EPOCHS = 10000
...
def main():
    ...
    torch.save({"X_train": X_tr, "y_train": y_tr, "X_test": X_te, "y_test": y_te},
               os.path.join(out_dir, "fig2_data.pt"))

    base_config = {
        "figure": "fig2", "example": "example1_sincos",
    ...
    for name, method_config, train_fn in methods:
        ...
        wb_run = wandb.init(
            ...
            name=f"fig2-{name}-{tag}",
            config=run_config,
            tags=["phase1", "fig2", "example1", tag, name],
            reinit=True,
        )
        ...
        fname = name.lower()
        torch.save(result, os.path.join(out_dir, f"fig2_{fname}.pt"))
    ...
    torch.save(all_results, os.path.join(out_dir, "fig2_all_results.pt"))

    # Plots
    plot_loss_curves(all_results, "Fig 2: Example 1 (D=40, m=1000, lr=0.2, λ=0)",
                     os.path.join(out_dir, "fig2_loss_curves.png"))
    sav_results = [r for r in all_results if "SAV" in r["method"]]
    plot_energy_curves(sav_results, "Fig 2: Example 1",
                       os.path.join(out_dir, "fig2_energy_curves.png"))
    plot_r_values(sav_results, "Fig 2: Example 1",
                  os.path.join(out_dir, "fig2_r_values.png"))

    total_time = time.time() - t_total
    print(f"\n{'='*60}")
    print(f"Fig 2 COMPLETE | Total wall time: {total_time:.1f}s")
    print(f"{'='*60}")
    ...
    send_slack(
        f"Phase 1 Fig 2 COMPLETE | {epochs} epochs | "

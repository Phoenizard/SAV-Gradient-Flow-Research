"""Phase 2 — ESAV on Example 2 (polynomial), λ=0.

Same data settings as Phase 1 Fig 7: D=40, m=100, M=10000, l=64, lr=0.01, λ=0
Methods: SGD (baseline), Vanilla ESAV, Restart ESAV, Relax ESAV

Usage:
    python experiments/phase2_example2.py              # full 10k epochs
    python experiments/phase2_example2.py --smoke      # smoke test (100 epochs)
"""
...
# Same settings as Phase 1 Fig 7
D = 40
M_TOTAL = 10000
M_NEURONS = 100
BATCH_SIZE = 64
LR = 0.01  # dt for ESAV, lr for SGD
LAMBDA = 0.0
FULL_EPOCHS = 10000
...
def main():
    ...
    base_config = {
        "phase": "phase2", "example": "example2_poly",
    ...
    for name, method_config, train_fn in methods:
        ...
        wb_run = wandb.init(
            ...
            name=f"p2-ex2-{name}-{tag}",
            config=run_config,
            tags=["phase2", "example2", tag, name],
            reinit=True,
        )
    ...
    plot_loss_curves(all_results,
                     "Phase 2 ESAV: Example 2 (D=40, m=100, lr=0.01, λ=0)",
                     os.path.join(out_dir, "example2_loss_curves.png"))
    esav_results = [r for r in all_results if "ESAV" in r["method"]]
    plot_energy_curves(esav_results, "Phase 2 ESAV: Example 2",
                       os.path.join(out_dir, "example2_energy_curves.png"))
    plot_r_values(esav_results, "Phase 2 ESAV: Example 2",
                  os.path.join(out_dir, "example2_r_values.png"))

    total_time = time.time() - t_total
    print(f"\n{'='*60}")
    print(f"Phase 2 Example 2 COMPLETE | Total wall time: {total_time:.1f}s")
    print(f"{'='*60}")
    ...
    send_slack(
        f"Phase 2 Example 2 COMPLETE | {epochs} epochs | "

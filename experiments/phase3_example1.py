"""Phase 3 — IEQ on Example 1 (sin+cos), λ=0.

Same data settings as Phase 1/2: D=40, m=1000, M=10000, batch_size=64, lr=0.2, λ=0
...
Usage:
    python experiments/phase3_example1.py              # full 5k epochs
    python experiments/phase3_example1.py --smoke      # smoke test (50 epochs)
"""
...
def main():
    ...
    base_config = {
        "phase": "phase3", "example": "example1_sincos",
    ...
    for name, method_config, train_fn in methods:
        ...
        wb_run = wandb.init(
            ...
            name=f"p3-ex1-{name}-{tag}",
            config=run_config,
            tags=["phase3", "example1", tag, name],
            reinit=True,
        )
    ...
    plot_loss_curves(all_results,
                     "Phase 3 IEQ: Example 1 (D=40, m=1000, lr=0.2, \u03bb=0)",
                     os.path.join(out_dir, "example1_loss_curves.png"))
    ieq_results = [r for r in all_results if "IEQ" in r["method"]]
    plot_energy_curves(ieq_results, "Phase 3 IEQ: Example 1",
                       os.path.join(out_dir, "example1_energy_curves.png"))
    plot_r_values(ieq_results, "Phase 3 IEQ: Example 1 (||q|| trajectory)",
                  os.path.join(out_dir, "example1_r_values.png"))

    total_time = time.time() - t_total
    print(f"\n{'='*60}")
    print(f"Phase 3 Example 1 COMPLETE | Total wall time: {total_time:.1f}s")
    print(f"{'='*60}")
    ...
    send_slack(
        f"Phase 3 Example 1 COMPLETE | {epochs} epochs | "

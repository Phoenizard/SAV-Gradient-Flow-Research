...
Training uses MSE loss; relative error is reported for paper comparison.
"""
...
def _compute_mu_and_loss(model, theta, X_batch, y_batch, C, train_loss_fn):
    """Compute mu^n = grad_I / sqrt(I + C) for a mini-batch.

    Sets model params from theta, does forward+backward using MSE loss.
    Returns (I_batch, mu, grad_I, r_true) where I_batch is the MSE loss value,
...
def train_vanilla_sav(model, X_train, y_train, X_test, y_test,
    ...
    # Initialize theta and r from full dataset (using MSE)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for idx in batches:
            ...
            # Step 1: compute mu (using MSE loss)
            I_batch, mu, grad_I, _ = _compute_mu_and_loss(
                model, theta, X_b, y_b, C, train_loss_fn
            )
        ...
        if wandb_run is not None:
            ...
            wandb_run.log({"epoch": epoch,
                           ...
                           "train_mse": tr_mse, "energy": energy, "r": r})
    ...
    send_slack(
        f"Vanilla SAV done | final_test_rel={test_losses[-1]:.4e} | "
...
def train_restart_sav(model, X_train, y_train, X_test, y_test,
    ...
    wall_time = time.time() - t0
    send_slack(
        f"Restart SAV done | final_test_rel={test_losses[-1]:.4e} | "
        f"wall_time={wall_time:.1f}s"
    )
...
def train_relax_sav(model, X_train, y_train, X_test, y_test,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for idx in batches:
            ...
            # === Vanilla step (semi-implicit, MSE loss) ===
            I_batch, mu, grad_I, _ = _compute_mu_and_loss(
                model, theta, X_b, y_b, C, train_loss_fn
            )
            a = torch.dot(mu, theta).item()
            ...
            # Step 2: evaluate I(theta_new) via forward pass (MSE)
            I_new = _evaluate_batch_loss(model, theta_new, X_b, y_b, train_loss_fn)
    ...
    wall_time = time.time() - t0
    send_slack(
        f"Relax SAV done | final_test_rel={test_losses[-1]:.4e} | "
        f"wall_time={wall_time:.1f}s"
    )
